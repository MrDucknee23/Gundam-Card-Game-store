  ...
  useEffect(() => {
    const fetchOrder = async () => {
      try {
        ...
        toast.error('Không thể tải chi tiết đơn hàng. Vui lòng kiểm tra kết nối mạng hoặc thử lại sau!');
        setOrder(null);
      } finally {
        setIsLoading(false);
      }
      ...
      <div className="min-h-screen bg-gray-50 p-6">
        <div className="max-w-7xl mx-auto">
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-12 text-center">
            ...
            <p className="text-gray-600 mb-6">Đơn hàng bạn đang tìm không tồn tại, đã bị xóa hoặc có lỗi kết nối.</p>
            <button onClick={() => window.location.reload()} className="mt-2 text-primary underline">Tải lại trang</button>
            <br />
            <Link
              to="/admin/orders"
              className="inline-flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-lg font-semibold hover:bg-primary/90 transition-colors mt-4"
            >
